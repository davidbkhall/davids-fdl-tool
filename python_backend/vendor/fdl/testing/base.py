...
class BaseFDLTestCase(unittest.TestCase):
    ...
    def get_resources_folder(cls):
        """Gets the FDL test resources folder from the fdl package tests directory."""
        # Navigate: fdl/testing/ -> fdl/ -> src/ -> packages/fdl/ -> tests/resources/FDL
        return Path(__file__).parents[3] / "tests" / "resources" / "FDL"
    ...
    def run_template_test(
        ...
        **kwargs : dict
        ...
        with patch("fdl.canvastemplate.uuid.uuid4", return_value=deterministic_uuid):
        ...
        # Verify FDL components using FDLComparison
        expected_canvas_id = deterministic_uuid.hex[:30]
        expected_fd_id = f"{expected_canvas_id}-{framing_intent_id}"

        self._fdl_comparator.compare_context(expected_context, result.context)
        self._fdl_comparator.compare_canvas(expected_canvas, result.canvas, expected_id=expected_canvas_id)
        self._fdl_comparator.compare_framing_decision(expected_fd, result.framing_decision, expected_id=expected_fd_id)
